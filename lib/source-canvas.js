...
export default class SourceCanvas {
  constructor(size) {
    ...
    loadImage(idomeneoText).then(image => {
      ...
      this.titleYPos = Math.floor(this.titleStartPos - window.pageXOffset / 8);
    });

    window.addEventListener('scroll', () => {
      this.titleYPos = this.titleStartPos - Math.floor(window.pageYOffset / 8);
    });
  }
  ...
  update(tick) {
    ...
    if (this.image) {
      this.ctx.drawImage(this.image, 0, this.titleYPos, this.size, this.size);
    }
  }
}